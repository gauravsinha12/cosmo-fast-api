...
from products import get_products_collection
...
async def list_orders(limit: int = Query(10, gt=0, le=100), offset: int = Query(0, ge=0)):
    orders_collection = await get_orders_collection()
...
async def update_order(order_id: str, updated_order: Order):
    ...
    await orders_collection.update_one({"_id": order_id_obj}, {"$set": updated_order.model_dump()})
    products_collection = await get_products_collection()
    for updated_item in updated_order.items:
        product_id = ObjectId(updated_item.productId)
        original_item = next((item for item in original_items if item["productId"] == str(product_id)), None)
        if original_item:
            quantity_difference = updated_item.boughtQuantity - original_item["boughtQuantity"]
            await products_collection.update_one(
                {"_id": product_id},
                {"$inc": {"quantity": -quantity_difference}}
            )
